...
contract MarginTradingRouter is AccessControl {
    ...
    constructor(address _marginAccountManager, address _marginTrading) {
    ...
    modifier onlyNotRestricted(address token) {
        require(
            provideWithdrawRestricted[token] == false,
            "Provide or withdraw is restricted"
        );
        _;
    }
    ...
    function setOneClickProxy(
        IOneClickProxy newOneClickProxy
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        oneClickProxy = newOneClickProxy;
    }

    function setProvideWithdrawRestricted(
        address token,
        bool value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function provideERC20(
        ...
        address token,
        uint amount
    ) external onlyApprovedOrOwner(marginAccountID) onlyNotRestricted(token) {
