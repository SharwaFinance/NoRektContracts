...
contract OneClickEphemeralSwapOutput is AccessControl {
    ...
    function setOriginalTokenToEphemeralToken(
        address original,
        address ephemeral
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function swapOutput(
        ...
        EphemeralERC20Type1(ephemeralToken).mintTo(
            address(this),
            amountInMaximum
        );
        oneClickProxy.provideERC20(
            marginAccountID,
            ephemeralToken,
            amountInMaximum
        );
        oneClickProxy.withdrawERC20NoYellow(
            marginAccountID,
            tokenOut,
            amountInMaximum
        );
        ...
        EphemeralERC20Type1(ephemeralToken).burnTo(
            address(this),
            amountInMaximum
        );
    }
    ...
    function getAmountIn(
        ...
        amountIn = quoter.quoteExactOutput(
            abi.encodePacked(tokenIn, POOL_FEE, tokenOut),
            amountOut
        );
    }
}
