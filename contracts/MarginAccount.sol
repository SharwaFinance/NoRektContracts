 ...
 * @author 0nika0
 */
contract MarginAccount is IMarginAccount, AccessControl, ReentrancyGuard {
    ...
    function getAvailableTokenToLiquidityPool()
        public
        view
        returns (address[] memory tokensArray)
    {
    ...
    function checkERC721tokenID(
        uint marginAccountID,
        address token,
        uint value
    ) public view returns (bool hasERC721Id) {
        uint[] memory userERC721 = new uint[](
            erc721ByContract[marginAccountID][token].length
        );
    ...
    function checkERC20Amount(
        uint marginAccountID,
        address token,
        uint amount
    ) external view returns (bool currectBalance) {
        currectBalance = amount <= erc20ByContract[marginAccountID][token];
    }

    function checkERC721Value(
        uint marginAccountID,
        address token,
        uint value
    ) external view returns (bool hasERC721Id) {
        hasERC721Id = checkERC721tokenID(marginAccountID, token, value);
    }

    function checkLiquidityPool(
        address token
    ...
    function setTokenToLiquidityPool(
        address token,
    ...
    function setAvailableTokenToLiquidityPool(
        address[] memory _availableTokenToLiquidityPool
    ) external onlyRole(MANAGER_ROLE) {
        availableTokenToLiquidityPool = _availableTokenToLiquidityPool;

        emit UpdateAvailableTokenToLiquidityPool(
            _availableTokenToLiquidityPool
        );
    }

    function setAvailableErc20(
        address[] memory _availableErc20
    ) external onlyRole(MANAGER_ROLE) {
    ...
    function setAvailableErc721(
        address[] memory _availableErc721
    ) external onlyRole(MANAGER_ROLE) {
    ...
    function provideERC20(
        ...
        address token,
        uint amount
    ) external nonReentrant onlyRole(MARGIN_TRADING_ROLE) {
        require(
            isAvailableErc20[token],
    ...
    function provideERC721(
        ...
        require(
            erc721ByContract[marginAccountID][token].length < erc721Limit,
            "erc721limit is exceeded"
        );
    ...
    function repay(
        ...
        uint debtWithAccruedInterest = ILiquidityPool(liquidityPoolAddress)
            .getDebtWithAccruedInterest(marginAccountID);
        if (amount == 0 || amount > debtWithAccruedInterest) {
            amount = debtWithAccruedInterest;
        }
        require(
            amount <= erc20ByContract[marginAccountID][token],
            "Insufficient funds to repay the debt"
        );
    ...
    function liquidate(
        ...
        IModularSwapRouter.ERC20PositionInfo[]
            memory erc20Params = new IModularSwapRouter.ERC20PositionInfo[](
                availableErc20.length
            );
        IModularSwapRouter.ERC721PositionInfo[]
            memory erc721Params = new IModularSwapRouter.ERC721PositionInfo[](
                availableErc721.length
            );

        for (uint i; i < availableErc20.length; i++) {
            uint erc20Balance = erc20ByContract[marginAccountID][
                availableErc20[i]
            ];
        ...
        for (uint i; i < availableErc721.length; i++) {
            uint[] memory erc721TokensByContract = erc721ByContract[
                marginAccountID
            ][availableErc721[i]];
        ...
        uint amountOutInUSDC = modularSwapRouter.liquidate(
            marginAccountID,
            erc20Params,
            erc721Params
        );
    ...
    function swap(
        ...
        require(
            isAvailableErc20[tokenIn] && isAvailableErc20[tokenOut],
            "Token is not available"
        );
        require(
            amountIn <= erc20ByContract[marginAccountID][tokenIn],
    ...
    function exercise(
        ...
        require(
            isAvailableErc721[erc721Token] && isAvailableErc20[baseToken],
            "Token is not available"
        );
    ...
    function _deleteERC721TokenFromContractList(
        ...
        uint[] storage userTokensByContract = erc721ByContract[marginAccountID][
            token
        ];

        for (uint i = 0; i < userTokensByContract.length; i++) {
            if (userTokensByContract[i] == tokenID) {
                userTokensByContract[i] = userTokensByContract[
                    userTokensByContract.length - 1
                ];
     ...
     */
    function _clearDebtsWithPools(
        uint marginAccountID,
        address baseToken,
        address liquidator
    ) private {
        for (uint i; i < availableTokenToLiquidityPool.length; i++) {
            address liquidityPoolAddress = tokenToLiquidityPool[
                availableTokenToLiquidityPool[i]
            ];
            uint poolDebt = ILiquidityPool(liquidityPoolAddress)
                .getDebtWithAccruedInterest(marginAccountID);
            if (poolDebt != 0) {
                uint amountInUSDC = modularSwapRouter.calculateAmountInERC20(
                    availableTokenToLiquidityPool[i],
                    baseToken,
                    poolDebt
                );
                uint userUSDCbalance = getErc20ByContract(
                    marginAccountID,
                    baseToken
                );
                if (amountInUSDC > userUSDCbalance) {
                    uint amountOutMinimum = modularSwapRouter
                        .calculateAmountOutERC20(
                            baseToken,
                            availableTokenToLiquidityPool[i],
                            userUSDCbalance
                        );
                    ...
                    erc20ByContract[marginAccountID][
                        baseToken
                    ] -= userUSDCbalance;
                    uint insurancePoolBalance = IERC20(
                        availableTokenToLiquidityPool[i]
                    ).balanceOf(insurancePool);
                    uint shortfall = amountOut >= poolDebt
                        ? 0
                        : poolDebt - amountOut;
                    if (insurancePoolBalance > shortfall) {
                        if (shortfall > 0) {
                            IERC20(availableTokenToLiquidityPool[i])
                                .transferFrom(
                                    insurancePool,
                                    address(this),
                                    shortfall
                        ...
                        emit InsurancePoolDepleted(
                            marginAccountID,
                            availableTokenToLiquidityPool[i],
                            poolDebt - amountOut,
                            insurancePoolBalance
                        );
                        poolDebt = amountOut + insurancePoolBalance;
                    }
                } else {
                ...
                ILiquidityPool(liquidityPoolAddress).repayForLiquidation(
                    marginAccountID,
                    poolDebt
                );
            }
        }
        uint userUSDCbalanceAfterRepay = getErc20ByContract(
            marginAccountID,
            baseToken
        );
        uint liquidatorCommission = (userUSDCbalanceAfterRepay *
            liquidatorFee) / COEFFICIENT_DECIMALS;
