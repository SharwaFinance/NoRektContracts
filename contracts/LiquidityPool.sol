    ...
    ReentrancyGuard
{
    ...
    bytes32 public constant PROTOCOL_ROUTER_ROLE =
        keccak256("PROTOCOL_ROUTER_ROLE");
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    ERC20 public immutable baseToken;
    ERC20 public immutable poolToken;

    uint public currentPoolBalance = 0;
    ...
    constructor(
        ...
        _grantRole(PROTOCOL_ROUTER_ROLE, _marginAccountStorage);
        _grantRole(PROTOCOL_ROUTER_ROLE, msg.sender);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    // ONLY MANAGER_ROLE FUNCTIONS //

    function revokeProtocolRouterRole(
        address account
    ) external onlyRole(MANAGER_ROLE) {
        _revokeRole(PROTOCOL_ROUTER_ROLE, account);
    }

    function setMaximumPoolCapacity(
    ...
    function setBlockNumberDelay(
        uint newBlockNumberDelay
    ) external onlyRole(MANAGER_ROLE) {
    ...
    function setInsuranceRateMultiplier(
        ...
        require(
            newInsuranceRateMultiplier <= 0.5 * 1e4,
            "The insurance rate multiplier cannot be more than 50%!"
        );
    ...
    // ONLY PROTOCOL_ROUTER_ROLE FUNCTIONS //

    function provide(
        uint amount
    ) external onlyRole(PROTOCOL_ROUTER_ROLE) nonReentrant {
        ...
        uint shareChange = totalLiquidity > 0
            ...
            : 1e18;
        require(shareChange > 0, "zero shares minted");
    ...
    function withdraw(
        ...
        require(
            currentPoolBalance >= amountWithdraw,
    ...
    function borrow(
        ...
        uint amount
    ) external onlyRole(PROTOCOL_ROUTER_ROLE) {
        ...
        require(
            borrows + amount <=
                ((borrows + currentPoolBalance) * maximumBorrowMultiplier) /
    ...
    function repay(
        ...
        require(
            borrowingBlockNumber[marginAccountID] + blockNumberDelay <=
                block.number,
            "The block number has not reached a value that allows to repay loan!"
        );
        _repay(marginAccountID, amount, msg.sender);
    }
    ...
    function _repay(
        ...
        uint amount,
        address sender
    ) private onlyRole(PROTOCOL_ROUTER_ROLE) {
        ...
        uint shareChange = debtSharesSum.mulDiv(
            amount,
            newTotalBorrows,
            Math.Rounding.Up
        );
        shareChange = shareChange > shareOfDebt[marginAccountID]
            ? shareOfDebt[marginAccountID]
            : shareChange;
        if (debt <= amount) {
            amount = debt;
        ...
        uint profit = (accruedInterest * shareChange) /
            shareOfDebt[marginAccountID];
        uint profitInsurancePool = (profit * insuranceRateMultiplier) /
            INTEREST_RATE_COEFFICIENT;
        if (totalInterestSnapshot > 0) {
            uint nowInterestSnapshot = Math.mulDiv(
                newTotalBorrows - netDebt - totalInterestSnapshot,
                shareOfDebt[marginAccountID],
                debtSharesSum,
                Math.Rounding.Up
            );
            uint sub = profit * shareOfDebt[marginAccountID];
            uint add = totalInterestSnapshot *
                shareOfDebt[marginAccountID] +
                nowInterestSnapshot *
                shareChange;
            totalInterestSnapshot = add >= sub
                ? (add - sub) / shareOfDebt[marginAccountID]
                : 0;
        }
        debtSharesSum -= shareChange;
        shareOfDebt[marginAccountID] -= shareChange;
        if (debt <= amount || shareOfDebt[marginAccountID] == 0) {
            netDebt -= portfolioIdToDebt[marginAccountID];
            portfolioIdToDebt[marginAccountID] = 0;
        } else {
            uint tempDebt = Math.mulDiv(
                portfolioIdToDebt[marginAccountID],
                debt - amount,
                debt,
                Math.Rounding.Up
            );
        ...
        totalInterestSnapshot = netDebt > 0 ? totalInterestSnapshot : 0;
        poolToken.transferFrom(sender, address(this), amount);
        currentPoolBalance += amount;
        if (profitInsurancePool > 0) {
            poolToken.transfer(insurancePool, profitInsurancePool);
            currentPoolBalance -= profitInsurancePool;
        }
    ...
    function getDebtWithAccruedInterestOnTime(
        ...
        UD60x18 temp = div(
            convert(
                ((INTEREST_RATE_COEFFICIENT + interestRate) * precision) /
                    INTEREST_RATE_COEFFICIENT
            ),
            convert(precision)
        );
        uint newTotalBorrow = ((netDebt + totalInterestSnapshot) *
            intoUint256(
                pow(
                    temp,
                    div(
                        convert(checkTime - totalBorrowsSnapshotTimestamp),
                        convert(ONE_YEAR_SECONDS)
                    )
                )
            )) / 1e18;
        uint debtWithAccruedInterest = (newTotalBorrow *
            shareOfDebt[marginAccountID]) / debtSharesSum;
    ...
    function getDebtWithAccruedInterest(
        uint marginAccountID
    ) public view returns (uint debtByPool) {
        if (debtSharesSum == 0) return 0;
        uint debtWithAccruedInterest = (totalBorrows() *
            shareOfDebt[marginAccountID]) / debtSharesSum;
        if (debtWithAccruedInterest < portfolioIdToDebt[marginAccountID]) {
            debtWithAccruedInterest = portfolioIdToDebt[marginAccountID];
        }
        ...
        return
            ((netDebt + totalInterestSnapshot) *
                intoUint256(
                    pow(
                        temp,
                        div(convert(ownershipTime), convert(ONE_YEAR_SECONDS))
                    )
                )) / 1e18;
    }
    ...
    function getTotalLiquidity() public view returns (uint) {
        ...
        uint insurancePoolLiquidity = ((nowTotalBorrows - netDebt) *
            insuranceRateMultiplier) / INTEREST_RATE_COEFFICIENT;
        return currentPoolBalance + nowTotalBorrows - insurancePoolLiquidity;
    }
