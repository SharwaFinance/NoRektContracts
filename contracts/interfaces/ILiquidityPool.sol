...
interface ILiquidityPool is IERC20 {
    ...
    /**
     * @notice Revokes the protocol router role from an account.
    ...
    /**
     * @notice Repays the debt for a margin account as part of a liquidation process.
     * @param marginAccountID The ERC-721 token ID representing the margin account.
     * @param amount The amount to repay denominated in poolToken.
     */
    function repayForLiquidation(uint marginAccountID, uint amount) external;
     ...
     */
    event UpdateMaximumPoolCapacity(uint newMaximumPoolCapacity);

    /**
     * @dev Emitted when the maximum borrow multiplier is updated.
     * @param newMaximumBorrowMultiplier The new maximum borrow multiplier.
     */
    event UpdateMaximumBorrowMultiplier(uint newMaximumBorrowMultiplier);

    /**
     * @dev Emitted when the insurance pool address is updated.
     * @param newInsurancePool The new address of the insurance pool.
     */
    event UpdateInsurancePool(address newInsurancePool);

    /**
     * @dev Emitted when the insurance rate multiplier is updated.
     * @param newInsuranceRateMultiplier The new insurance rate multiplier.
     */
    event UpdateInsuranceRateMultiplier(uint newInsuranceRateMultiplier);
     ...
     */
    event Borrow(uint indexed marginAccountID, uint amountDebtTokens);
