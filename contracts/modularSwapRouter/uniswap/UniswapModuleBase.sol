...
abstract contract UniswapModuleBase is IPositionManagerERC20, AccessControl {
    ...
    function setPath(bytes memory _path) external onlyRole(MANAGER_ROLE) {
        path = _path;
    }
    ...
    function getPositionValue(
        uint256 amountIn
    ) external virtual returns (uint amountOut);
    ...
    function liquidate(
        ...
        ERC20(tokenInContract).transferFrom(
            marginAccount,
            address(this),
            amountIn
        );

        ISwapRouter.ExactInputParams memory params = prepareInputParams(
            amountIn
        );
    ...
    function swapOutput(
        uint amountOut
    ) external onlyRole(MODULAR_SWAP_ROUTER_ROLE) returns (uint amountIn) {
        ISwapRouter.ExactOutputParams memory params = prepareOutputParams(
            amountOut
        );
        ...
        ERC20(tokenOutContract).transferFrom(
            marginAccount,
            address(this),
            amountIn
        );
